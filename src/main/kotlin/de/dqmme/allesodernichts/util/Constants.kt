package de.dqmme.allesodernichts.util

import net.axay.kspigot.extensions.pluginKey

object Constants {

    val ARMOR_KEY = pluginKey("armor")

    const val WORLDBORDER_SIZE = 300.0
    const val TIME = 720

    const val MAX_POINTS = 750
    const val MILESTONE_BROADCAST_PERCENTAGE = 0.1

    const val POINTS_PER_MATERIAL = 5
    const val POINTS_PER_ANIMAL = 20
    const val POINTS_PER_ADVANCEMENT = 15
    const val POINTS_PER_TASK = 150

    const val MATERIAL_POINTS_STEP = 1
    const val ANIMAL_POINTS_STEP = 5
    const val ADVANCEMENT_POINTS_STEP = 5
    const val TASK_POINTS_STEP = 50


    const val DEATH_POINT_LOSS_PERCENTAGE = 0.5
    const val KILL_POINT_GAIN_PERCENTAGE = 0.1

    const val ELON_MUSK = "fick dein marsprojekt"
}
